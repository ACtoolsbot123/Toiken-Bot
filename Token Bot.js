        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'stock_modal') {
                try {
                    ...
                    return interaction.editReply({
                        content: `📦 **Token added!** Total tokens: \`${tokenStock.length}\``
                    });
                } catch (err) {
                    console.error('[TMC.LOL] Stock Modal Error:', err);
                    return interaction.editReply({ content: '❌ **Error:** Failed to process token.' });
                }
            }
            ...
            if (interaction.customId === 'redeem_modal') {
                ...
                if (validCodes.has(code)) {
                    ...
                    if (!supporterRole) {
                        return interaction.editReply({ content: `🎉 **Code Validated!** But the Supporter role wasn't found.` });
                    }
                    try {
                        await interaction.member.roles.add(supporterRole);
                        return interaction.editReply({ content: `🎉 **Redemption Successful!** Code \`${code}\` verified!` });
                    } catch (err) {
